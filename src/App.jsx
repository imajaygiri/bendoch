...
import { 
  Terminal, Cpu, Zap, Wifi, Battery, Command, 
  GitBranch, Folder, FileCode, User, Server, 
  Activity, X, Minus, Square, ChevronRight, 
  Hash, Code, Power, Github, Linkedin, Mail, 
  ExternalLink, Box, Instagram, Globe, Twitter
} from 'lucide-react';
  ...
  <div className="flex justify-between items-center bg-[#1a1b26] border-t border-[#414868] text-xs font-mono select-none">
    <div className="flex">
      <div className={`px-3 py-1 font-bold ${
        mode === 'NORMAL' ? 'bg-blue-600 text-black' : 
        mode === 'INSERT' ? 'bg-green-600 text-black' : 'bg-purple-600 text-black'
      }`}>
        {mode}
      </div>
      ...
      <div className="px-3 py-1 bg-[#1f2335] text-[#7aa2f7]">
        {file}
      </div>
    </div>
    ...
    <div className="flex-1 overflow-y-auto font-mono text-sm">
      {INITIAL_STATE.files.map(file => (
        <div 
          ...
          className={`px-4 py-2 cursor-pointer flex items-center gap-2 hover:bg-[#292e42] ${
            activeFile === file.name ? 'bg-[#292e42] text-[#7aa2f7]' : 'text-[#565f89]'
          }`}
        >
          {file.icon}
          {file.name}
      ...
      <div className="mt-4 px-4 text-[#414868] text-xs uppercase font-bold">External Deps</div>
      <div className="px-4 py-1 text-[#565f89] flex items-center gap-2 text-sm"><Hash size={12}/> node_modules</div>
      <div className="px-4 py-1 text-[#565f89] flex items-center gap-2 text-sm"><Hash size={12}/> target (rust)</div>
      <div className="px-4 py-1 text-[#565f89] flex items-center gap-2 text-sm"><Hash size={12}/> .git</div>
    </div>
  </div>
);
...
const FounderCard = ({ founder }) => (
  <div className="bg-[#24283b] border border-[#414868] rounded p-4 font-mono hover:border-[#7aa2f7] transition-all group relative overflow-hidden flex flex-col h-full">
    <div className="absolute top-0 right-0 p-2 opacity-20 group-hover:opacity-100 transition-opacity">
      {founder.os.includes("Arch") ? <Cpu className="text-blue-400" /> : <Command className="text-gray-400" />}
    </div>
    ...
    <div className="mt-4 flex flex-wrap gap-2">
      {founder.skills.map(skill => (
        <span key={skill} className="px-1.5 py-0.5 bg-[#1f2335] text-[#9ece6a] text-[10px] rounded border border-[#414868]">
          {skill}
        </span>
      ))}
    </div>
    ...
    <div className="mt-4 pt-3 border-t border-[#414868] flex gap-3">
        {founder.socials.github && (
            <a href={founder.socials.github} target="_blank" rel="noreferrer" title="GitHub" className="text-[#565f89] hover:text-[#c0caf5] transition-colors">
                <Github size={16} />
            </a>
        )}
        {founder.socials.linkedin && (
            <a href={founder.socials.linkedin} target="_blank" rel="noreferrer" title="LinkedIn" className="text-[#565f89] hover:text-[#7aa2f7] transition-colors">
                <Linkedin size={16} />
            </a>
        )}
        {founder.socials.instagram && (
            <a href={founder.socials.instagram} target="_blank" rel="noreferrer" title="Instagram" className="text-[#565f89] hover:text-[#f472b6] transition-colors">
                <Instagram size={16} />
            </a>
        )}
        {founder.socials.twitter && (
            <a href={founder.socials.twitter} target="_blank" rel="noreferrer" title="Twitter/X" className="text-[#565f89] hover:text-white transition-colors">
                <Twitter size={16} />
            </a>
        )}
        {founder.socials.website && (
            <a href={founder.socials.website} target="_blank" rel="noreferrer" title="Website" className="text-[#565f89] hover:text-[#9ece6a] transition-colors">
                <Globe size={16} />
            </a>
        )}
        {founder.socials.email && (
            <a href={founder.socials.email} title="Email" className="text-[#565f89] hover:text-[#e0af68] transition-colors">
                <Mail size={16} />
            </a>
        )}
    </div>
...
const ProjectsBlock = ({ projects }) => (
  <div className="font-mono text-sm p-4">
    ...
    <div className="space-y-6">
      <div className="text-[#bb9af7]">package <span className="text-[#c0caf5]">main</span></div>
      ...
      <div className="text-[#bb9af7]">var <span className="text-[#c0caf5]">Portfolio</span> = []<span className="text-[#e0af68]">Project</span>{'{'}</div>
      
      <div className="pl-4 space-y-4">
        {projects.map((proj) => (
            <div key={proj.id} className="group hover:bg-[#292e42] p-2 rounded cursor-pointer -ml-2 border border-transparent hover:border-[#414868] transition-all">
                <div className="text-[#e0af68]">{'{'}</div>
                <div className="pl-4">
                    <span className="text-[#9aa5ce]">Name:</span> <span className="text-[#9ece6a]">"{proj.name}"</span>,
                </div>
                {/* ADDED: Author Field */}
                <div className="pl-4">
                    <span className="text-[#9aa5ce]">Author:</span> <span className="text-[#f7768e]">"{proj.author}"</span>,
                </div>
                <div className="pl-4">
                    <span className="text-[#9aa5ce]">Stack:</span> <span className="text-[#7aa2f7]">"{proj.lang}"</span>,
                </div>
                <div className="pl-4 flex items-center gap-2">
                    <span className="text-[#9aa5ce]">Desc:</span> <span className="text-[#bb9af7]">"{proj.desc}"</span>
                    <ExternalLink size={12} className="opacity-0 group-hover:opacity-100 text-[#7aa2f7]"/>
                </div>
                <div className="text-[#e0af68]">{'}'},</div>
            </div>
        ))}
...
const ServicesBlock = () => (
  <div className="font-mono text-sm p-4">
    ...
    <div className="space-y-6">
      {[
        { fn: "low_level_systems", type: "Kernel::Module", desc: "Drivers, Embedded, GDB Customization" },
        { fn: "web_infrastructure", type: "React::NextJS", desc: "Full Stack, Scalable Architectures" },
        { fn: "artificial_intelligence", type: "Model::Training", desc: "Python, PyTorch, Optimization" },
        { fn: "mobile_native", type: "Android::Kotlin", desc: "Native Apps, Cross-Platform Integration" }
      ].map((srv) => (
        <div key={srv.fn} className="pl-4 border-l border-[#292e42] hover:bg-[#1f2335] p-2 transition-colors">
          <span className="text-[#bb9af7]">fn</span> <span className="text-[#7aa2f7]">{srv.fn}</span>() {'->'} <span className="text-[#e0af68]">{srv.type}</span> {'{'}
          <div className="pl-4 text-[#9ece6a]">
            return "{srv.desc}";
          </div>
          {'}'}
        </div>
...
export default function BendochTerminal() {
  ...
  const handleCommand = (e) => {
    if (e.key === 'Enter') {
      ...
      switch(cmd) {
        case 'ls': response = 'README.md  team.json  projects.go  services.rs  contact.sh'; break;
        case 'whoami': response = 'guest@bendoch-corp'; break;
        case 'clear': setTerminalOutput([]); setCommand(''); return;
        case 'sudo rm -rf /': response = 'Permission denied. Nice try, script kiddie.'; break;
        case 'cat team.json': setActiveTab('team'); response = 'Opening buffer: team.json...'; break;
        case 'cat projects.go': setActiveTab('projects'); response = 'Opening buffer: projects.go...'; break;
        case 'cat services.rs': setActiveTab('services'); response = 'Opening buffer: services.rs...'; break;
        case 'neofetch': 
            setActiveTab('home'); 
            response = 'Executing neofetch...'; 
            break;
        case 'help': response = 'Available commands: ls, cat, neofetch, clear'; break;
  ...
  return (
    <div className="h-screen w-full bg-[#1a1b26] text-[#a9b1d6] flex flex-col font-mono overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <FileTree 
            activeFile={
                activeTab === 'home' ? 'README.md' : 
                activeTab === 'team' ? 'team.json' : 
                activeTab === 'projects' ? 'projects.go' :
                activeTab === 'services' ? 'services.rs' : 'contact.sh'
            }
          ...
          <div className="flex bg-[#16161e] overflow-x-auto">
            {INITIAL_STATE.files.map(f => (
                <div 
                    ...
                    className={`px-4 py-2 text-sm border-r border-[#1f2335] cursor-pointer flex items-center gap-2 whitespace-nowrap ${
                        activeTab === f.content ? 'bg-[#24283b] text-[#c0caf5]' : 'text-[#565f89]'
                    }`}
                >
                    {f.icon} {f.name} {activeTab === f.content && <X size={12} className="ml-2 hover:text-red-400"/>}
                </div>
            ))}
          </div>
...
`}
                    ...
                    <div className="space-y-2">
                        <h1 className="text-3xl text-[#c0caf5] font-bold mb-4">Bendoch Corp <span className="text-[#565f89] text-lg">@localhost</span></h1>
                        <p><span className="text-[#bb9af7] font-bold">OS:</span> Custom Linux/Unix Hybrid</p>
                        <p><span className="text-[#bb9af7] font-bold">Kernel:</span> Software Engineering v6.0</p>
                        <p><span className="text-[#bb9af7] font-bold">Mission:</span> "Bringing Customer to Us"</p>
                        
                        <div className="flex gap-4 mt-6">
                            <a href="https://github.com/imlearning2024" target="_blank" rel="noreferrer" className="px-4 py-2 bg-[#1f2335] border border-[#414868] text-[#7aa2f7] rounded flex items-center gap-2 hover:bg-[#292e42] transition-colors">
                                <Github size={16}/> GitHub
                            </a>
                            <a href="mailto:sinexcosecx@gmail.com" className="px-4 py-2 bg-[#1f2335] border border-[#414868] text-[#9ece6a] rounded flex items-center gap-2 hover:bg-[#292e42] transition-colors">
            ...
            {/* PROJECTS TAB (NEW) */}
            {activeTab === 'projects' && <ProjectsBlock projects={INITIAL_STATE.projects} />}
                ...
                <div className="font-mono text-sm max-w-2xl">
                    ...
                    <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                        <div>
                            ...
                            <input type="email" className="w-full bg-transparent border-b border-[#414868] focus:border-[#7aa2f7] outline-none py-1 text-[#c0caf5]" placeholder="user@client.com" />
                        </div>
                        <div>
                            <p className="text-[#bb9af7]">echo "Project Requirements:"</p>
                            <textarea rows={4} className="w-full bg-[#1f2335] mt-2 border border-[#414868] rounded p-2 text-[#c0caf5] focus:border-[#7aa2f7] outline-none" placeholder="rm -rf my_competitors..."></textarea>
                        </div>
          ...
          <div className="h-32 bg-[#1a1b26] border-t border-[#414868] p-2 overflow-y-auto font-mono text-sm">
            {terminalOutput.map((line, i) => (
                <div key={i} className="text-[#a9b1d6]">{line}</div>
            ))}
      ...
      {/* 3. STATUS BAR */}
      <StatusBar mode={activeTab === 'contact' ? 'INSERT' : 'NORMAL'} file={activeTab === 'home' ? 'readme.md' : activeTab} />
